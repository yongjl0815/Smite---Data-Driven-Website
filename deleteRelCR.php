<?php
	...
	//to check if this relationship exist
	if ($out_id == NULL){
		echo "That Relationship doesn't exist.";
	}
	else{	
		if (!($stmt = $mysqli->prepare("DELETE FROM 275CharRole WHERE cid=? AND rid=?"))) {
			echo "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error;
		}

		if (!$stmt->bind_param("ii", $Characterid, $Roleid)) {
